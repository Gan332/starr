...
#   & ([scriptblock]::Create((irm https://raw.githubusercontent.com/can1357/oh-my-pi/main/scripts/install.ps1))) -Local
#   & ([scriptblock]::Create((irm https://raw.githubusercontent.com/can1357/oh-my-pi/main/scripts/install.ps1))) -Local D:\path\to\omp.exe
...
function Install-Binary {
    ...
    # Check if destination is locked by a running omp process
    if (Test-Path $OutPath) {
        $originalHash = (Get-FileHash $OutPath -Algorithm MD5).Hash
    } else {
        $originalHash = $null
    }

    if ($LocalPath) {
        Write-Host "Installing from local binary: $LocalPath"
        if (-not (Test-Path $LocalPath)) {
            throw "Local binary not found: $LocalPath"
        }

        # Check if the local file is the same as what's already installed
        $localHash = (Get-FileHash $LocalPath -Algorithm MD5).Hash
        if ($localHash -eq $originalHash) {
            Write-Host "✓ Local binary matches installed version. Nothing to update." -ForegroundColor Green
            return
        }

        if (Test-FileLocked $OutPath) {
            Write-Host ""
            Write-Host "⚠ $OutPath is locked by a running omp process." -ForegroundColor Yellow
            Write-Host "  Close omp first, or run: Stop-Process -Name omp -Force" -ForegroundColor Yellow
            Write-Host ""
            throw "Cannot overwrite — omp is running."
        }
        ...
        # Download binary — Invoke-WebRequest will overwrite regardless of lock
...
# Main logic
if ($Local) {
    Install-Binary -LocalPath $Local
} elseif ($Source) {
    ...
    # Default: look for local binary first, download as fallback
    $localCandidates = @(Join-Path $PWD "omp.exe"), (Join-Path $PWD $BinaryName)
    $localFound = $null
    foreach ($candidate in $localCandidates) {
        if (Test-Path $candidate) {
            $localFound = $candidate
            break
        }
    }

    if ($localFound) {
        Write-Host "Found local binary: $localFound" -ForegroundColor Cyan
        Install-Binary -LocalPath $localFound
    } else {
        Install-Binary
    }
}
